...
var save = function(endCursor) {
    chrome.storage.local.get(null, function(items) {  
        if (items.instaUserID && items.totalMedia) {
            
            var userName = (items.instaUserName ? items.instaUserName : 'Instagram User');
            var mediaAmount = 50;                      
            var instaQueryHash = '42323d64886122307be10013ad2dcc44';
            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    InstaSaver.addToMedia(data.data.user.edge_owner_to_timeline_media.edges);
                    var pageInfo = data.data.user.edge_owner_to_timeline_media.page_info;

                    if (pageInfo.has_next_page) {                           
                        save(pageInfo.end_cursor);
                    }
                    else {  
